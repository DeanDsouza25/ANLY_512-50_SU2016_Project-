...
```{r,warning=FALSE,message=FALSE,echo=FALSE}
p2<-ggplot(data=exptemp, aes(x=Date,y=Expenditure,colour=Category))+geom_line()+facet_wrap(~Category)
p2
```
...
p4<-ggplot(data = hrstemp, aes(x=Date,y=Hours.Spent, colour=Category))+geom_line()+facet_wrap(~Category)
(gg<-ggplotly(p4))
```

  3. The third question can be better visualized with the help of a facet wrapped scatterplot as follows:  
